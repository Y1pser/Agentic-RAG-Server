...
| A2 | 引入 pytest 并建立测试目录约定 | [ ] | — | pytest 配置、`tests/unit\|integration\|e2e\|fixtures` 目录、至少 1 个冒烟测试 |
| A3 | 配置加载与校验（Settings） | [ ] | — | `config/settings.yaml` 最小可解析配置 + `Settings` dataclass + 必填字段校验 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1.1 | LLM 抽象接口与工厂 | [ ] | — | `BaseLLM` + `LLMFactory`，支持按配置选择 provider |
